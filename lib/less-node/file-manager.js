...
FileManager.prototype.loadFile = function(filename, currentDirectory, options, environment, callback) {
    ...
    if (options.paths) {
        ...
        paths.push.apply(paths, isAbsoluteFilename ? [""] : [currentDirectory]);
        ...
        (function tryPathIndex(i) {
            if (i < paths.length) {
                ...
                if (paths[i]) {
                    ...
                    //console.info('['+i+'] '+fullFilename);
                } else {
                    //console.log('Index ' + i + ' ignored');
                }
                fs.stat(fullFilename, function (err) {
                    if (err) {
                        //console.error('NOT FOUND @ '+fullFilename+'!');
                        filenamesTried.push(fullFilename);
                        tryPathIndex(i + 1);
                    } else {
                        //console.error('FOUND @ '+fullFilename+'!');
                        fs.readFile(fullFilename, 'utf-8', function(e, data) {
                            if (e) { reject(e); return; }
                            if (options.debugImports) data = "\n/* Imported from "+fullFilename+" */\n";
